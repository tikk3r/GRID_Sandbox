...
function make_plots(){
   ...
   #find ${RUNDIR}/prefactor/*result* -name "*.png" -exec cp {} ${JOBDIR} \;
   # Remove pngs in prefactor docs and Input so they don't get picked up
   find ${JOBDIR}/prefactor/ -name "*.png" -exec rm {} \;
   find ${JOBDIR}/Input/ -name "*.png" -exec rm {} \;
   find ${RUNDIR}/Output/  -name "*.png" -exec cp {} ${JOBDIR} \;
