...
sing_img_path=/cvmfs/softdrive.nl/oonk/SINGULARITY_IMAGES/C7_2_20_2_sml_env/lofar-2_20_2_c7_sml_env.simg
singularity exec --pwd $PWD ${sing_img_path} NDPPP -h
echo ""
# -----------------------------------------------------------------------
...
singularity exec --pwd ${PWD} ${sing_img_path} which genericpipeline.py
echo ${PARSET}
ls pipeline.cfg
...
singularity exec --pwd ${PWD} ${sing_img_path} echo $PYTHONPATH

singularity exec --pwd ${PWD} ${sing_img_path} genericpipeline.py ${PWD}/${PARSET} -d -c pipeline.cfg > output
#
